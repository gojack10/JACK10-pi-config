...
type Phase = "armed" | "injected" | "checkpoint" | "forced";
type State = {
	version: 1;
	phase: Phase;
	threshold: number;
...
export function registerFriendlyStop(pi: ExtensionAPI, env: Env = process.env): boolean {
	...
	const markInjected = () => {
		...
		state = { ...state, phase: "injected" };
		persist();
		return true;
	};
	const sendCheckpointTurn = (ctx: ExtensionContext) => {
		if (!markInjected()) return;
		pi.sendMessage({ customType: STATE_TYPE, content: INSTRUCTION, display: true }, { triggerTurn: true, deliverAs: "followUp" });
		show(ctx, "RLM checkpoint turn requested", "warning");
	};
	...
	pi.on("session_start", (_event, ctx) => {
		...
		if (!state && arm(ctx)) sendCheckpointTurn(ctx);
		else if (state?.phase === "armed") sendCheckpointTurn(ctx);
		else if (state && state.phase !== "checkpoint" && state.phase !== "forced") show(ctx, "RLM rollover wrap-up active", "warning");
	});
	pi.on("session_shutdown", () => { shuttingDown = true; });
	pi.on("turn_end", (_event, ctx) => { if (!shuttingDown) arm(ctx); });
	pi.on("context", async (event, ctx) => {
		if (shuttingDown || !state || state.phase === "checkpoint" || state.phase === "forced") return;
		if (state.wrapupTurns >= enabled.graceTurns) {
			await forceStop(ctx);
			return;
		}
		state = { ...state, wrapupTurns: state.wrapupTurns + 1 };
		const inject = markInjected();
		if (!inject) persist();
		if (!inject) return;
		return {
	...
	pi.on("agent_settled", (_event, ctx) => {
		if (!shuttingDown && state?.phase === "armed") sendCheckpointTurn(ctx);
	});
